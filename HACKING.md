...
The checked-in presets split ordinary builds from developer tooling:

- `release` is the portable build/install preset.
- `dev` adds formatting, linting, cppcheck, tests, and `compile_commands.json`.

On Windows, developer checks that rely on `compile_commands.json` still require
a Ninja-based profile, plus `clang-format` and `clang-tidy` on `PATH`.
